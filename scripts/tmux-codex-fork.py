...
def fork_plan(tmux, pane, codex):
    ...
    if result.returncode or not thread:
        raise ValueError('This pane has no Codex thread ID; refusing to guess from its directory')
    uuid.UUID(thread)
    return [*tmux, 'new-window', '-t', session + ':', '-n', 'fork-' + thread[:8],
            '-c', cwd, shlex.join([codex, 'fork', thread])]
